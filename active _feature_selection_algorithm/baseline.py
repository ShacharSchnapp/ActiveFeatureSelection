...
from baselines.coreset.coreset import feature_ranking_coreset
from baselines.random.random import feature_ranking_random
from baselines.svm_active.svm_active import svm_active
...
def main():
    ...
    parser.add_argument("-label", metavar='TOTAL_LABELS', default=100,
    ...
    parser.add_argument("-n_threads", metavar='NUMBER_OF_THREADS', default=1,
    ...
    for i in range(t_times):
        ...
        for p in pole:
            if p == 'RANDOM':
                ...
                result.append([mi_score(X, y, k, prob_x, np.arange(afs.n_samples))] * total_labels)
            elif p == 'CORESET':
                result.append(feature_ranking_coreset(X, y, k, prob_x, total_labels))
            elif p == 'SVM-ACTIVE':
                result.append(svm_active(X, y, k, prob_x, total_labels))
            elif p == 'AFS':
                result.append(afs.run(total_labels, plato=lam))
...
def run_all_experiment(X, y, total_labels, k, data_name, delta, n_threads, t_times, lam):
    ...
    pole = ['RANDOM', 'AFS', 'ALL-SAMPLE', 'CORESET', 'SVM-ACTIVE']
    process = []

...
from afs.afs import AFS, mi_score
    ...
    for i in range(t_times):
        ...
        for p in pole:
            if p == 'RANDOM':
                ...
                result.append(afs.run(total_labels, method='single', agg_func=None))
            elif p == 'AVG-ALL':
                result.append(afs.run(total_labels, agg_func=None, method='all'))
            elif p == 'AVG-SEL':
                result.append(afs.run(total_labels, agg_func=None))
            elif p == 'AFS-PLAIN':
                result.append(afs.run(total_labels, plato=False))
            elif p == 'AFS':
...
def run_all_experiment(X, y, total_labels, k, data_name, delta, n_threads, t_times, lam):
    pole = ['RANDOM', 'SINGLE', 'AVG-ALL', 'AVG-SEL', 'ALL-SAMPLE', 'AFS-PLAIN', 'AFS']
